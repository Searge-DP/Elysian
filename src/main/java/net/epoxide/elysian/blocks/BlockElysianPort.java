...
public class BlockElysianPort extends Block {
    ...
    public boolean onBlockActivated (World world, int x, int y, int z, EntityPlayer player, int meta, float p_149727_7_, float p_149727_8_, float p_149727_9_) {
        ...
        if (!world.isRemote)
            FMLCommonHandler.instance().getMinecraftServerInstance().getConfigurationManager().transferPlayerToDimension((EntityPlayerMP) player, Constants.DIM_ID, new TeleporterElysian(MinecraftServer.getServer().worldServerForDimension(Constants.DIM_ID)));
