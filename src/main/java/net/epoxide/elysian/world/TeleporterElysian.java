...
public class TeleporterElysian extends Teleporter {
	...
	@Override
	public void placeInPortal(Entity entity, double posX, double posY,
			double posZ, float yaw) {
				...
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						this.worldServerInstance.setBlock(k1, l1, i2,
								flag ? Blocks.obsidian : Blocks.air);
					}
				}
			}
	...
	@Override
	public boolean placeInExistingPortal(Entity entity, double posX,
		...
		if (this.destinationCoordinateCache.containsItem(j1)) {
			TeleporterElysian.PortalPosition portalposition = (TeleporterElysian.PortalPosition) this.destinationCoordinateCache
					.getValueByKey(j1);
			d3 = 0.0D;
					...
					for (int i2 = this.worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2) {
						...
						if (this.worldServerInstance.getBlock(l3, i2, l1) == BlockHandler.transporter) {
							while (this.worldServerInstance.getBlock(l3,
									i2 - 1, l1) == BlockHandler.transporter) {
								--i2;
							}
		...
		if (d3 >= 0.0D) {
			if (flag) {
				this.destinationCoordinateCache.add(j1,
						new TeleporterElysian.PortalPosition(i, j, k,
			...
			entity.setLocationAndAngles(d11, d6, d7, entity.rotationYaw,
