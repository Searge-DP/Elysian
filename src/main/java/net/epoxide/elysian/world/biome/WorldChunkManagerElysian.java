...
public class WorldChunkManagerElysian extends WorldChunkManager {
	...
	@Override
	public float[] getRainfall(float[] listToReuse, int x, int z, int width,
			int length) {
		...
		for (int i1 = 0; i1 < width * length; ++i1) {
			try {
				...
				crashreportcategory.addCrashSection("downfalls[] size",
						Integer.valueOf(listToReuse.length));
				crashreportcategory.addCrashSection("x", Integer.valueOf(x));
				crashreportcategory.addCrashSection("z", Integer.valueOf(z));
				crashreportcategory
						.addCrashSection("w", Integer.valueOf(width));
				crashreportcategory.addCrashSection("h",
	...
	@Override
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] biomesArray,
		...
		try {
			...
			crashreportcategory.addCrashSection("biomes[] size",
					Integer.valueOf(biomesArray.length));
	...
	@Override
	public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] listToReuse, int x,
			int y, int width, int length, boolean cacheFlag) {
		...
		if (cacheFlag && width == 16 && length == 16 && (x & 15) == 0
				&& (y & 15) == 0) {
			BiomeGenBase[] abiomegenbase1 = this.biomeCache.getCachedBiomes(x,
					y);
	...
	@Override
	public ChunkPosition findBiomePosition(int x, int y, int z, List par4List,
			Random random) {
