...
public class CopyOfChunkProviderElysian implements IChunkProvider {

	private Random rand;
	private World worldObj;
	private MapGenBase elysianCaveGenerator = new MapGenCavesHell(); // update :
																		// this
																		// has
																		// nothing
																		// to do
	...
	{
		elysianCaveGenerator = TerrainGen.getModdedMapGen(elysianCaveGenerator,
				NETHER_CAVE);
	}

	public CopyOfChunkProviderElysian(World world, long seed) {
	...
	public void prepareChunk(int chunkX, int chunkZ, Block[] chunkBlocks) {
		...
		byte b2 = 17;
		int l = b0 + 1;
		this.noiseField = this.initializeNoiseField(this.noiseField, chunkX
				* b0, 0, chunkZ * b0, k, b2, l);
	...
	// TODO map variables
	public void replaceBiomeBlocks(int chunkX, int chunkZ, Block[] chunkBlocks,
			byte[] chunkMetas, BiomeGenBase[] possibleBiomes) {
		...
		biome = (BiomeGenElysian) base; // TODO biome only reflects one biome,
										// instead of thsoe
										// we have, altough the world detects
										// both biomes
		...
		for (int y = 0; y < 16; ++y) {
			for (int x = 0; x < 16; ++x) {
				int minusOne = -1;
				Block block = biome.fillerBlock; // this block is above sea
													// level
				Block block1 = biome.fillerBlock; // this block is under sea
													// level

				for (int posY = 127; posY >= 0; --posY) { // top down to ground
															// level
					int blockPos = (x * 16 + y) * 128 + posY;

					if (posY < 127 - this.rand.nextInt(5)
							&& posY > 0 + this.rand.nextInt(5)) { // all
																	// blocks
																	// between
																	// heightlimit
																	// and
						...
						if (block2 != null) {
							if (block2.getMaterial() != Material.air) { // if
																		// not
																		// air
								if (block2 == biome.fillerBlock) {
									if (minusOne == -1) {

										if (posY >= waterLevel - 4
												&& posY <= waterLevel + 1) {
											block = biome.fillerBlock;
									...
									} else if (minusOne > 0) { // does this even
																// ever get
																// checked
																// ??
										--minusOne;
	...
	public Chunk provideChunk(int posX, int posZ) {
		...
		this.rand.setSeed((long) posX * 341873128712L + (long) posZ
				* 132897987541L);
		Block[] chunkBlocks = new Block[32768];
		byte[] chunkMetas = new byte[chunkBlocks.length];
		this.prepareChunk(posX, posZ, chunkBlocks);
		this.biomesForGeneration = this.worldObj.getWorldChunkManager()
				.loadBlockGeneratorData(this.biomesForGeneration, posX * 16,
						posZ * 16, 16, 16);
		this.replaceBiomeBlocks(posX, posZ, chunkBlocks, chunkMetas,
				this.biomesForGeneration);
		this.elysianCaveGenerator.func_151539_a(this, this.worldObj, posX,
				posZ, chunkBlocks);
		Chunk chunk = new Chunk(this.worldObj, chunkBlocks, chunkMetas, posX,
				posZ);

		this.worldObj.getWorldChunkManager().loadBlockGeneratorData(
				(BiomeGenBase[]) null, posX * 16, posZ * 16, 16, 16);
	...
	private double[] initializeNoiseField(double[] noiseField, int posX,
			int posY, int posZ, int sizeX, int sizeY, int sizeZ) {

		// TODO remove forge events
		// ChunkProviderEvent.InitNoiseField event = new
		// ChunkProviderEvent.InitNoiseField(this, noiseField, posX, posY, posZ,
		// sizeX, sizeY,
		// sizeZ);
		...
		double d0 = 684.412D;
		double d1 = 2053.236D;
		this.noiseDataD = this.noiseD.generateNoiseOctaves(this.noiseDataD,
				posX, posY, posZ, sizeX, 1, sizeZ, 1.0D, 0.0D, 1.0D);
		this.noiseDataE = this.noiseE.generateNoiseOctaves(this.noiseDataE,
				posX, posY, posZ, sizeX, 1, sizeZ, 100.0D, 0.0D, 100.0D);
		this.noiseDataA = this.noiseC.generateNoiseOctaves(this.noiseDataA,
				posX, posY, posZ, sizeX, sizeY, sizeZ, d0 / 80.0D, d1 / 60.0D,
				d0 / 80.0D);
		this.noiseDataB = this.noiseA.generateNoiseOctaves(this.noiseDataB,
				posX, posY, posZ, sizeX, sizeY, sizeZ, d0, d1, d0);
		this.noiseDataC = this.noiseB.generateNoiseOctaves(this.noiseDataC,
				posX, posY, posZ, sizeX, sizeY, sizeZ, d0, d1, d0);
		int k1 = 0;
		int l1 = 0;
	...
	public void populate(IChunkProvider chunkProvider, int posX, int posY) {
		...
		MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Pre(chunkProvider,
				worldObj, rand, posX, posY, false));
		...
		MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Post(
				chunkProvider, worldObj, rand, posX, posY, false));
	...
	@Override
	public List getPossibleCreatures(EnumCreatureType creatureType, int posX,
			int posY, int posZ) {

		return this.worldObj.getBiomeGenForCoords(posX, posZ).getSpawnableList(
				creatureType);
	}

	@Override
	public ChunkPosition func_147416_a(World world, String structureName,
