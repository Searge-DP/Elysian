...
public class MapGenCaveElysian extends MapGenBase {
	...
	protected void func_151543_a(long p_151543_1_, int p_151543_3_,
		...
		for (boolean flag = random.nextInt(6) == 0; p_151543_15_ < p_151543_16_; ++p_151543_15_) {
			double d6 = 1.5D + (double) (MathHelper.sin((float) p_151543_15_
			...
			if (!flag1 && p_151543_15_ == k1 && p_151543_12_ > 1.0F) {
				this.func_151543_a(random.nextLong(), p_151543_3_, p_151543_4_,
						p_151543_5_, p_151543_6_, p_151543_8_, p_151543_10_,
						random.nextFloat() * 0.5F + 0.5F, p_151543_13_
			...
			if (flag1 || random.nextInt(4) != 0) {
				...
				if (p_151543_6_ >= d4 - 16.0D - d6 * 2.0D
						&& p_151543_10_ >= d5 - 16.0D - d6 * 2.0D
						&& p_151543_6_ <= d4 + 16.0D + d6 * 2.0D
						&& p_151543_10_ <= d5 + 16.0D + d6 * 2.0D) {
					int i4 = MathHelper.floor_double(p_151543_6_ - d6)
							- p_151543_3_ * 16 - 1;
					int l1 = MathHelper.floor_double(p_151543_6_ + d6)
							- p_151543_3_ * 16 + 1;
					int j4 = MathHelper.floor_double(p_151543_8_ - d7) - 1;
					int i2 = MathHelper.floor_double(p_151543_8_ + d7) + 1;
					int k4 = MathHelper.floor_double(p_151543_10_ - d6)
							- p_151543_4_ * 16 - 1;
					int j2 = MathHelper.floor_double(p_151543_10_ + d6)
							- p_151543_4_ * 16 + 1;
					...
					if (!flag2) {
						for (k2 = i4; k2 < l1; ++k2) {
							double d13 = ((double) (k2 + p_151543_3_ * 16) + 0.5D - p_151543_6_)
									/ d6;

							for (j3 = k4; j3 < j2; ++j3) {
								double d14 = ((double) (j3 + p_151543_4_ * 16) + 0.5D - p_151543_10_)
										/ d6;
								int k3 = (k2 * 16 + j3) * 128 + i2;

								for (int l3 = i2 - 1; l3 >= j4; --l3) {
									double d12 = ((double) l3 + 0.5D - p_151543_8_)
											/ d7;
									...
									if (d12 > -0.7D
										...
										if (isTopBlock(p_151543_5_, k3, k2, l3,
												j3, p_151543_3_, p_151543_4_)) {
											flag1 = true;
										}
										digBlock(p_151543_5_, k3, k2, l3, j3,
												p_151543_3_, p_151543_4_, flag1);
									}
		...
		for (int j1 = 0; j1 < i1; ++j1) {
			...
			for (int l1 = 0; l1 < k1; ++l1) {
				...
				this.func_151543_a(this.rand.nextLong(), p_151538_4_,
						p_151538_5_, p_151538_6_, d0, d1, d2, f2 * 2.0F, f, f1,
						0, 0, 0.5D);
			}
		}
	}
