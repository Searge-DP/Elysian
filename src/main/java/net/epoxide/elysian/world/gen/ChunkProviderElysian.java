...
public class ChunkProviderElysian implements IChunkProvider {
	...
	/**
	 * Determines whether lateriteGrass or porphyry can be generated at a
	 * location
	 */
	private NoiseGeneratorOctaves lateriteGrassPorphyryNoise;

	/**
	 * Determines whether something other than porphyry can be generated at a
	 * location
	 */
	...
	/**
	 * Holds the noise used to determine whether lateriteGrass can be generated
	 * at a location
	 */
	...
	/**
	 * Holds the noise used to determine whether something other than porphyry
	...
	public ChunkProviderElysian(World par1World, long par2) {
		...
		this.lateriteGrassPorphyryNoise = new NoiseGeneratorOctaves(this.rand,
				4);
		this.porphyryExclusivityNoiseGen = new NoiseGeneratorOctaves(this.rand,
				4);
		...
		NoiseGenerator[] noiseGens = { netherNoiseGen1, netherNoiseGen2,
				netherNoiseGen3, lateriteGrassPorphyryNoise,
				porphyryExclusivityNoiseGen, netherNoiseGen6, netherNoiseGen7,
				mobSpawnerNoise };
	...
	public void generateNetherTerrain(int posX, int posZ, Block[] abyte) {
		...
		BiomeGenBase biomegenbase = this.worldObj.getBiomeGenForCoords(k + 16,
				l + 16); // TODO marked here for noticing the +16

		// BiomeGenBase biomegenbase = this.worldObj.getBiomeGenForCoords(posX,
		// posZ);
		...
		if (biome == null) {
			System.out.println("skipped " + k + " " + l
					+ " chunk because biome wasnt our elysian biome");
			return;
		}
		...
		this.biomesForGeneration = this.worldObj.getWorldChunkManager()
				.getBiomesForGeneration(this.biomesForGeneration, posX * 4 - 2,
						posZ * 4 - 2, k + 5, l + 5);

		this.noiseField = this.initializeNoiseField(this.noiseField, posX * b0,
				0, posZ * b0, k, b2, l);
	...
	public void replaceBlocksForBiome(int chunkX, int chunkZ,
			Block[] chunkBlocks, BiomeGenBase[] possibleBiomes) {
	...
	public Chunk provideChunk(int par1, int par2) {
		...
		// BiomeGenBase[] abiomegenbase = (BiomeGenBase[])
		// this.worldObj.getWorldChunkManager().loadBlockGeneratorData((BiomeGenBase[])null,
	...
	public void decorate(World par1World, Random par2Random, int par3, int par4) {
		((BiomeGenBase) this.theBiomeDecorator).decorate(par1World, par2Random,
				par3, par4);
	}
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Pre(
				par1IChunkProvider, worldObj, rand, par2, par3, false));
		...
		boolean doGen = TerrainGen.populate(par1IChunkProvider, worldObj, rand,
				par2, par3, false, NETHER_LAVA);

		// doGen = TerrainGen.populate(par1IChunkProvider, worldObj, soulRNG,
		// par2, par3, false, DUNGEON);
		...
		// SoulBlocks.Porphyry.get())).generate(worldObj, soulRNG, randPosX,
		// randPosY, randPosZ);
		// }
		//
		// for (int i = 0; i < 5; i++)
		...
		// int randPosY = soulRNG.nextInt(128);
		// int randPosZ = l + soulRNG.nextInt(16);
		// (new WorldGenMinable(SoulBlocks.SoulSnowBottom.get(), 40,
		// SoulBlocks.SoulSnowTop.get())).generate(worldObj, soulRNG, randPosX,
		// randPosY, randPosZ);
		// }
	...
	public List getPossibleCreatures(EnumCreatureType par1EnumCreatureType,
		...
		BiomeGenBase biomegenbase = (BiomeGenBase) this.worldObj
				.getBiomeGenForCoords(par2, par4);
